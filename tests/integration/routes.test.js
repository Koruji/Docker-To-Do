...
describe('Routes', () => {
  ...
  describe('GET /tasks/completed', () => {
    it('should retrieve completed tasks', async () => {
      const response = await request(app).get('/tasks/completed').expect(200);
  ...
  describe('GET /tasks/:id', () => {
    ...
    it('should return 404 for non-existent task', async () => {
      ...
      await request(app).get(`/tasks/${nonExistentId}`).expect(404);
    });
  });

  describe('PUT /tasks/:id/complete', () => {
    it('should complete a task', async () => {
      const response = await request(app).put(`/tasks/${taskId}/complete`).expect(200);
    ...
    it('should return 404 for non-existent task', async () => {
      const nonExistentId = 9999;
      await request(app).put(`/tasks/${nonExistentId}/complete`).expect(404);
    });
  });
